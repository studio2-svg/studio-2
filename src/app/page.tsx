/* eslint-disable @next/next/no-img-element -- Homepage media is selected from the managed CMS. */
...
export default async function Home() {
  ...
  if (!page)
    return (
      <main className="flex min-h-screen items-center justify-center bg-ink px-6 text-paper">
        <p>Connect Supabase and apply migrations to load published content.</p>
      </main>
    );
  const cta = page.content.primary_cta as
    { label?: string; url?: string } | undefined;
  const copy = (page.content.home_sections || {}) as Record<string, string>;
  ...
  return (
    <main className="bg-paper text-ink">
      <div className="relative bg-cover bg-center text-paper" style={{backgroundColor:hero.overlay_color||"#12110f",backgroundImage:hero.background_url?`url(${hero.background_url})`:undefined,color:hero.text_color||"#f3f0e9"}}>
        <div className="absolute inset-0" style={{backgroundColor:hero.overlay_color||"#12110f",opacity:(hero.overlay_opacity??65)/100}}/>
        <div className="relative"><PublicNav dark /></div>
        <section className="relative mx-auto grid min-h-[78vh] max-w-7xl content-end px-6 pb-16 lg:grid-cols-[1fr_22rem] lg:gap-20 lg:px-10 lg:pb-24">
          <div>
            <p className="mb-7 text-xs uppercase tracking-[.32em] text-gold">
              {page.subtitle}
            </p>
            <h1 className="max-w-4xl font-display text-6xl leading-[.88] sm:text-8xl lg:text-[8.5rem]">
              {page.title}
            </h1>
          </div>
          <div className="mt-10 self-end border-t border-white/20 pt-6">
            <p className="leading-7 text-white/65">{page.description}</p>
            <Link
              href={cta?.url || "/book"}
              className="mt-8 inline-flex items-center gap-2 text-sm uppercase tracking-[.2em] text-gold"
            >
              {cta?.label || "Start a booking"} <ArrowUpRight size={17} />
            </Link>
          </div>
        </section>
      </div>
      <section className="mx-auto grid max-w-7xl gap-12 px-6 py-24 lg:grid-cols-[1fr_1.3fr]">
        <p className="text-xs uppercase tracking-[.25em] text-gold">
          {copy.intro_eyebrow}
        </p>
        <div className="transition duration-300 hover:translate-x-2">
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 grid gap-px bg-black/10 md:grid-cols-3">
            {services.slice(0, 6).map((item) => (
              <article key={item.id} className="bg-[#e7e1d6] p-7 transition duration-300 hover:-translate-y-1 hover:bg-paper hover:shadow-xl">
      ...
      <section className="mx-auto max-w-7xl px-6 py-24">
        <Heading
          ...
          title="Tools chosen for the work."
          href="/equipment"
        />
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {equipment.slice(0, 3).map((item) => (
            <article key={item.id} className="group transition duration-300 hover:-translate-y-1">
              {item.image_url ? (
              ...
              <p className="mt-4 text-xs uppercase tracking-[.18em] text-gold">
                {item.equipment_categories?.name || "Equipment"}
              </p>
            ...
            <div className="mt-12 grid gap-5 md:grid-cols-2">
              {gallery.slice(0, 4).map((item) => (
                ...
                >
                  ...
                  <span className="absolute bottom-4 left-4 bg-ink px-3 py-2 text-xs uppercase tracking-[.15em] text-paper">
                    {item.category}
                  </span>
                </article>
              ))}
      ...
      {testimonials.length > 0 && (
        <section className="bg-[#e7e1d6] px-6 py-24">
          <div className="mx-auto max-w-5xl text-center">
            ...
            <blockquote className="mt-8 font-display text-4xl leading-tight sm:text-6xl">
              “{testimonials[0].review}”
            </blockquote>
            <p className="mt-7 text-sm">
              {testimonials[0].customer_name}
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-6 py-24 lg:grid-cols-2">
        <div>
          ...
          <h2 className="mt-4 font-display text-6xl">
            Good questions deserve clear answers.
          </h2>
          <Link
            href="/faq"
            className="mt-8 inline-block text-sm uppercase tracking-[.18em]"
          >
            See all FAQs →
          </Link>
        </div>
      ...
      <section className="bg-gold px-6 py-24 text-ink">
        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-8 md:flex-row md:items-end">
          <h2 className="max-w-3xl font-display text-6xl leading-none sm:text-7xl">
            Bring the idea. We’ll help build the production.
          </h2>
          <Link
            href="/book"
            className="inline-flex items-center gap-2 border border-ink px-6 py-4 text-sm uppercase tracking-[.18em]"
          >
            Book Studio Two <ArrowUpRight size={17} />
          </Link>
        </div>
      </section>
      <PublicFooter />
  ...
  return (
    <div className="flex items-end justify-between gap-5">
      <div>
        ...
        <h2 className="mt-4 font-display text-5xl sm:text-6xl">{title}</h2>
      </div>
      <Link
        href={href}
        className={`hidden text-sm uppercase tracking-[.15em] sm:block ${dark ? "text-white/60" : "text-black/55"}`}
      >
        View all →
      </Link>
    </div>
