                ...
                rentals.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 text-right"><details className="relative inline-block text-left"><summary className="cursor-pointer list-none border border-ink px-3 py-2">Edit</summary><div className="absolute right-0 z-30 mt-2 w-[32rem] max-w-[80vw] bg-paper p-5 text-left shadow-2xl"><ActionForm action={updateRentalOrder} successMessage="Rental updated." className="grid gap-3"><input type="hidden" name="id" value={r.id}/><TextField name="starts_at" label="Starts" type="datetime-local" value={new Date(r.starts_at).toISOString().slice(0,16)} required/><TextField name="ends_at" label="Ends" type="datetime-local" value={new Date(r.ends_at).toISOString().slice(0,16)} required/><TextField name="quantity" label="Quantity" type="number" value={r.quantity} required/><OrderSelect name="status" label="Rental status" value={r.status} options={["awaiting_payment","paid","ready","collected","returned","cancelled"]}/><OrderSelect name="payment_status" label="Payment status" value={rentalPayments?.find(p=>p.entity_id===r.id)?.status||"pending"} options={["pending","paid","failed","refunded"]}/><SaveButton label="Save rental"/></ActionForm></div></details></td>
                    </tr>
                  );
                })
              ) : (
