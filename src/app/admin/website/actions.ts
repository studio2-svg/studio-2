...
const pageSchema = z.object({
  ...
  ogDescription: z.string().trim().max(250),
  ogImageUrl: z.union([z.literal(""), z.url()]),
  robots: z.enum(["index,follow", "noindex,follow", "noindex,nofollow"]),
  intent: z.enum(["draft", "publish", "archive"]),
  heroBackgroundUrl: z.string().optional().default(""),
  heroOverlayColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional().default("#12110f"),
  heroOverlayOpacity: z.coerce.number().min(0).max(100).optional().default(65),
  heroTextColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional().default("#f3f0e9"),
});
export async function savePage(
  ...
  if (
    ...
    !profile.permissions.includes("website")
  )
    return { error: "You do not have permission to edit the website." };
  const status =
  ...
  const heroBackgroundUrl=input.slug==="home"?await uploadImage(admin,user.id,formData.get("heroBackground"),input.heroBackgroundUrl):input.heroBackgroundUrl;
  const content = {
    ...((current?.content as Record<string, unknown>) || {}),
    body: input.body,
    ...(input.slug==="home"?{hero:{background_url:heroBackgroundUrl,overlay_color:input.heroOverlayColor,overlay_opacity:input.heroOverlayOpacity,text_color:input.heroTextColor}}:{}),
  };
