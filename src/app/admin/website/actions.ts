...
export async function savePage(_: CmsActionState, formData: FormData): Promise<CmsActionState> {
  ...
  const { supabase, user, profile } = await requireAdmin(); const input = parsed.data;
  if (!["admin", "owner"].includes(profile.role) && !profile.permissions.includes("website")) return { error: "You do not have permission to edit the website." };
  ...
  const { data: saved, error } = await admin.from("cms_pages").update({ title: input.title, subtitle: input.subtitle || null, description: input.description || null, content, status, seo_title: input.seoTitle || null, seo_description: input.seoDescription || null, canonical_url: input.canonicalUrl || null, og_title: input.ogTitle || null, og_description: input.ogDescription || null, og_image_url: input.ogImageUrl || null, robots: input.robots, published_at: status === "published" ? new Date().toISOString() : null, updated_by: user.id }).eq("id", input.id).select("id,status,updated_at").single();
  if (error || !saved) return { error: error?.message || "The page was not saved. Please try again." };
