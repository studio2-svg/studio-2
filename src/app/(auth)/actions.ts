...
export async function register(_: AuthState, formData: FormData): Promise<AuthState> { const parsed = registerSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) return { error: "Complete every field with valid details." }; const { email, password, firstName, lastName } = parsed.data; const origin=(await headers()).get("origin")||"https://studio-2-psi.vercel.app"; const { error } = await (await createClient()).auth.signUp({ email, password, options: { emailRedirectTo:`${origin}/auth/callback?next=/dashboard`,data: { first_name: firstName, last_name: lastName } } }); if (error) return { error: error.message }; redirect("/login?registered=1"); }
