...
export function ServicesManager({ items }: { items: Service[] }) {
  const [editing, setEditing] = useState<Service | null | "new">(null),
    [viewing, setViewing] = useState<Service | null>(null);
  return (
    <>
            ...
            {items.map((item) => (
              ...
              >
                <td className="p-4">
                  ...
                  <small className="block text-black/45">/{item.slug}</small>
                </td>
                <td className="p-4">
                  GHS {(item.price_minor / 100).toFixed(2)} ·{" "}
                  {item.pricing_type}
                </td>
                <td className="p-4">{item.active ? "Active" : "Hidden"}</td>
                <td className="p-4">
      ...
      {editing && (
        <Modal
          title={editing === "new" ? "Add service" : `Edit ${editing.name}`}
          close={() => setEditing(null)}
        >
          <ServiceForm item={editing === "new" ? undefined : editing} />
        </Modal>
      )}
      {viewing && (
        <Modal title={viewing.name} close={() => setViewing(null)}>
          <div className="space-y-4">
            ...
            <p>
              <strong>GHS {(viewing.price_minor / 100).toFixed(2)}</strong> ·{" "}
              {viewing.pricing_type}
            </p>
          </div>
        </Modal>
      )}
...
function ServiceForm({ item }: { item?: Service }) {
  return (
    <ActionForm
      action={saveService}
      successMessage={item ? "Service updated." : "Service created."}
    ...
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <TextField
          ...
          label="Price (GHS)"
          type="number"
          value={item ? item.price_minor / 100 : 0}
        />
        <label className="text-sm">
          <span className="mb-2 block">Pricing</span>
          <select
            name="pricing_type"
            defaultValue={item?.pricing_type || "fixed"}
            className="w-full border border-black/15 bg-white p-3"
          >
            {["hourly", "daily", "fixed", "percentage", "custom"].map((x) => (
      ...
      <SaveButton label={item ? "Save changes" : "Create service"} />
    </ActionForm>
  );
}
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center overflow-y-auto bg-black/55 p-4">
      <section
        role="dialog"
        aria-modal="true"
        className="my-8 w-full max-w-2xl bg-paper p-6 shadow-2xl"
      >
...
function Icon({
  ...
  return (
    <button
      aria-label={label}
      onClick={onClick}
      className="grid size-9 place-items-center border border-black/15 transition hover:border-gold hover:bg-gold"
    >
      {children}
    </button>
  );
