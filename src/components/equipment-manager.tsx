...
import { deleteEquipment, saveEquipment } from "@/app/admin/equipment/actions";
...
export function EquipmentManager({
  ...
  const [editing, setEditing] = useState<Item | null | "new">(null),
    [viewing, setViewing] = useState<Item | null>(null);
  return (
    <>
              ...
              >
                ...
                <td className="p-4">
                  <div className="flex justify-end gap-2">
                    <Icon onClick={() => setViewing(item)}>
                      <Eye size={16} />
                    </Icon>
                    <Icon onClick={() => setEditing(item)}>
                      <Pencil size={16} />
                    </Icon>
                    <ActionForm
      ...
      {editing && (
        <Modal
          title={editing === "new" ? "Add equipment" : `Edit ${editing.name}`}
...
function Form({ item, categories }: { item?: Item; categories: Category[] }) {
  return (
    <ActionForm
      action={saveEquipment}
      successMessage={item ? "Equipment updated." : "Equipment created."}
    ...
    >
      ...
      />
      <TextArea name="specifications" label="Specifications" value={item?.specifications} />
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="text-sm">
          ...
          <select
            ...
            className="w-full border border-black/15 bg-white p-3"
          >
            {["hourly", "daily", "fixed", "included"].map((x) => (
        ...
        <label className="text-sm">
          ...
          <select
            ...
            className="w-full border border-black/15 bg-white p-3"
          >
            {["available", "unavailable", "maintenance", "retired"].map((x) => (
              <option key={x}>{x}</option>
            ))}
          </select>
        </label>
      ...
      <SaveButton label={item ? "Save changes" : "Create equipment"} />
    </ActionForm>
  );
}
function Modal({
