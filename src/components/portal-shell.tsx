...
export function PortalShell({
  ...
  const backHref = pathname.startsWith("/admin/website/")
    ? "/admin/website"
    : root;
  const navigation = (
    <>
      <p className="mb-6 text-xs uppercase tracking-[.22em] text-black/45">
        {title}
      </p>
      <nav className="space-y-1">
        {nav.map((item) => (
          <Link
            ...
            key={item.href}
            href={item.href}
            className={`block border-l px-4 py-2.5 text-sm ${pathname === item.href ? "border-gold bg-white" : "border-black/15 hover:border-gold hover:bg-white/50"}`}
          >
            {item.label}
          </Link>
        ))}
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6">
          <div className="flex items-center gap-3">
            <button
              ...
              className="grid size-10 place-items-center border border-black/15 md:hidden"
            >
              <Menu size={20} />
            </button>
          ...
          <aside className="relative h-full w-[min(19rem,86vw)] overflow-y-auto bg-paper px-6 py-6 shadow-2xl">
            <div className="mb-10 flex items-center justify-between">
              ...
              <button
                ...
                aria-label="Close navigation"
                onClick={() => setOpen(false)}
                className="grid size-10 place-items-center border border-black/15"
              >
                <X size={20} />
              </button>
            </div>
