...
export function CreateAdminForm() {
  ...
  return (
    <form action={action} className="mt-6 grid gap-5 sm:grid-cols-2">
      ...
      <button
        disabled={pending}
        className="bg-ink px-5 py-4 text-sm uppercase tracking-[.18em] text-paper disabled:opacity-50 sm:col-span-2"
      >
  ...
  return (
    <label className="block text-sm">
      ...
      <input
        ...
        minLength={minLength}
        autoComplete="off"
        className="w-full border border-black/20 bg-white px-4 py-3 outline-none focus:border-gold"
      />
    </label>
  );
}
